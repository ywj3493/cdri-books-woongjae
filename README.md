...
FSD 아키텍쳐를 이용해 폴더 구조를 구성하여 다음과 같이 3 depth 로 나누어 관리 합니다. 첫번째 depth는 규모에 따라 다음과 같이 6개의 폴더로 나누어 관리하며 각 규모 폴더 내부에는 관련 도메인인 `books`, `search`, `favorites` 로 나누어 하위에 목적에 맞는 폴더를 만들어 구분 합니다.
